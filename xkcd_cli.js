...
var nightvale = {
	...
	get: function(path, success, error) {
		data = {'title': path, 'img': this.base+path+"?"+this.count.toString()};
		this.count++;
		success(data);
	}
};

var nightValeDisplay = TerminalShell.commands['archive_search'] = function(terminal, path) {
	function fail() {
		terminal.print($('<p>').addClass('error').text('display: unable to open image "'+path+'": No such file or directory.'));
		terminal.setWorking(false);
	}
	...
	terminal.setWorking(true);
	nightvale.get(path, function(data) {
		$('<img>')
			.hide()
			.load(function() {
			...
			.attr({src:data.img, alt:data.title})
			.addClass('comic');
	}, fail);
};
...
TerminalShell.commands['shutdown'] = TerminalShell.commands['poweroff'] = function(terminal) {
	if (this.sudo) {
		terminal.print('Broadcast message from fey@WZZZ');
		terminal.print();
...
Filesystem = {
	'welcome.txt': {type:'file', read:function(terminal) {
		terminal.print($('<h4>').text('Welcome to the WZZZ Station Console.'));
		terminal.print('          ******       ******');
		terminal.print('        **********   **********');
		terminal.print('      ************* *************');
		terminal.print('     *****************************');
...
Filesystem['archive_search'] = 'archive_search';
...
$(document).ready(function() {
	...
	$('#screen').bind('cli-load', function(e) {
		xkcd.get(null, function(data) {
			if (data) {
				...
				Terminal.runCommand('archive_search cecil');
			} else {
				noData();
			}
		}, noData);
	...
	$(document).konami(function(){
		...
		if (konamiCount == 0) {
			...
			$('#screen').css('background', 'url(/over9000.jpg) center no-repeat');
		}
